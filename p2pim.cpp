...
void sendTCPMessage(int type, int fd) {
    ...
    *((uint16_t*)(outgoingTCPMsg + 4)) = htons(type);
	...
	if(findClientByFd(fd)->second.connectionType != 1)
	{
        int wrLen = 0, j = 0;
        while(wrLen < 6) {
            j = write(fd, outgoingTCPMsg, 6 - wrLen);
            wrLen += j;
        }
            ...
            switch(type) {
                ...
                case REPLY_USER_LIST: {
                    ...
                    for(int k = 0; k < entryCount; k++) {
                        ...
                        recvLen = 0, j = 0;
                        while(j < 2) { 
                            recvLen = read(it->fd, entryArr + 6 + newClient.hostName.length() + 1, 2 - j);
                            j += recvLen;
                        }
				...
				case ENCRYPTED_DATA_CHUNK_MESSAGE: {
                    ...
                    switch(newType) {
                        ...
                        case FILE_DATA_MESSAGE:{
                            ...
                            int wrLen = 0, j = 0;
                            while(wrLen < dataSize) {
                                j = write(findClientByFd(it->fd)->second.fileReceivingFd, encryptedDataChunk + 14, dataSize - wrLen);
                                wrLen += j;
                            }
